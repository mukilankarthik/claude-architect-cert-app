...
- **`materials/*.json`** — structured content rendered in the Materials tab, each loaded by its own `@st.cache_data` loader and its own `elif section == ...` branch in the materials render block. `cheat_sheets.json` / `cram_sheet.json` / `claude_code_reference.json` / `glossary.json` are per-domain or per-topic reference; `mental_model.json` is the Five Actors + 8-step agentic loop explainer; `scenario_walkthroughs.json` keys its entries by the exact `name` values in `SCENARIOS` (app.py) and traces one worked example per named exam scenario; `capstone.json` is a standalone project brief. Adding a new JSON-backed section means: a loader function, an entry in `get_materials_groups()`, and a render branch — follow the existing ones as the template.
